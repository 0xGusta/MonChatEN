...
export default function Tetris({ players, sessionId, myAddress, onGameEnd }) {
    ...
    const [gameState, setGameState] = useStateTogether(`tetris-gamestate-${sessionId}`, {
        P1: { board: createEmptyBoard(), score: 0, level: 1, lines: 0, gameOver: false },
        P2: { board: createEmptyBoard(), score: 0, level: 1, lines: 0, gameOver: false },
    ...
    const updatePlayerPos = (newPos, newShape) => {
        setPlayer(prev => ({ ...prev, pos: newPos, shape: newShape || prev.shape }));
    };

    const resetPlayer = useCallback(() => {
        const nextShapeIndex = gameState.pieceSequence[pieceIndex];
    ...
    useEffect(() => {
        resetPlayer();
    }, [gameState.P1.board, gameState.P2.board]);
    ...
    const playerRotate = (dir) => {
        ...
        let offset = 1;
        while (checkCollision(clonedPlayer)) {
    ...
    const movePlayer = (dir) => {
        if (!checkCollision({ ...player, pos: { x: player.pos.x + dir, y: player.pos.y } })) {
            setPlayer(prev => ({ ...prev, pos: { x: prev.pos.x + dir, y: prev.pos.y } }));
        }
    };

    const dropPlayer = () => {
        if (!checkCollision({ ...player, pos: { x: player.pos.x, y: player.pos.y + 1 } })) {
    ...
    const checkCollision = (playerPiece) => {
    ...
    const addGarbageLines = (linesCleared) => {
        const garbageCount = linesCleared - 1;
        if (garbageCount <= 0) return;

        setGameState(prev => {
            const newOpponentBoard = prev[opponentSymbol].board.slice();
    ...
    const drawBoard = (canvasRef, board, currentPlayer = null) => {
        ...
        board.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value !== 0) {
                    ctx.fillStyle = value === 8 ? '#6B7280' : COLORS[value];
